...
export let formTemplate = (form) => html`
    <form @submit=${form.submitHandler} class=${ifDefined(form.class)} id=${form.id} ${form.type === 'edit' 
...
export let allBooksTemplate = (books,editHandler)=> html`
   ${books.map(b => bookTemplate(b,editHandler))}
`;
